...
(async () => {
  ...
  try {
    ...
    await page.goto(
      pathToFileURL(path.resolve(__dirname, "../dist/renderer/index.html"))
        .href,
    );
    ...
    assert(
      ...
      "Theme control must fit in the settings header",
    );
    const ratioBox = await page.locator(".ratio-grid").boundingBox();
    assert(
      ratioBox.y + ratioBox.height <= 800,
      "Drawing parameters must fit in the initial desktop viewport",
    );
    ...
    await page
      .getByRole("option", { name: "最近 10 条对话", exact: true })
      .click();
    await page.reload();
    await page.waitForSelector(".session-item");
    ...
    assert(
      await page
        .locator('.res-btn[data-val="4K"]')
        .evaluate((el) => el.classList.contains("active")),
    );
    await page.locator('.ratio-card[data-val="16:9"]').click();
    assert(
      await page
        .locator('.ratio-card[data-val="16:9"]')
        .evaluate((el) => el.classList.contains("active")),
    );
    await page.locator("#auto-save-toggle").click();
    assert(
      !(await page.locator("#auto-save-toggle").isChecked()),
      "Auto-save must still require a directory",
    );
    await page.locator("#check-update-btn").click();
    assert(
      await page.locator("#update-status-text").isVisible(),
      "Update errors must remain visible",
    );
    assert.equal(
      await page.locator("#update-status-text").textContent(),
    ...
    for (const width of [1280, 390, 320]) {
      ...
      for (const mode of ["light", "dark"]) {
        ...
        assert(
          sidebar.x >= 0 && sidebar.x + sidebar.width <= width,
          "Settings must fit the viewport",
        );
        if (process.env.SETTINGS_SCREENSHOTS) {
          await page.screenshot({
            path: path.join(
              process.env.SETTINGS_SCREENSHOTS,
              `settings-${width}-${mode}.png`,
            ),
            animations: "disabled",
          });
        }
    ...
    assert.equal(
      await settingsBtn.getAttribute("title"),
      null,
      "The settings icon must not show a tooltip over the chat",
    );
    assert.equal(await settingsBtn.getAttribute("aria-label"), "设置");
    assert((await page.locator("#right-sidebar").boundingBox()).width > 200);
    ...
    assert.equal(
      await page
        ...
        .evaluate((el) => el.classList.contains("active")),
      false,
      "Desktop settings must not cover the chat",
    );
    const input = await page.locator("#user-input").boundingBox();
    assert.equal(
    ...
    await page.locator("#settings-toggle-mobile").click();
    assert.equal(
      await page
        .locator("#right-sidebar")
        .evaluate((el) => el.classList.contains("open")),
    ...
    console.log(
      "Settings checks passed: compact layout, themes, persistence, controls, update errors, desktop/mobile overflow.",
    );
  } finally {
    await browser.close();
  }
