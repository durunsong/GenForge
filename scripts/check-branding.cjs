...
test("Electron keeps existing profiles and sets the GenForge identity before ready", () => {
  ...
  const cases = [
    ...
    {
      existing: ["GenForge", "Gemini绘图工作台"],
      packaged: true,
      expected: "GenForge",
    },
    {
      existing: ["Gemini绘图工作台"],
      packaged: true,
      expected: "Gemini绘图工作台",
    },
    {
      existing: ["gemini-image-studio"],
      packaged: false,
      expected: "gemini-image-studio",
    },
    {
      existing: ["gemini-image-studio"],
      packaged: true,
    ...
    {
      existing: ["Gemini绘图工作台", "gemini-image-studio"],
      packaged: false,
      expected: "gemini-image-studio",
    },
  ];
    ...
    vm.runInNewContext(main, {
      ...
      require(id) {
        ...
        if (id === "fs")
          return {
            existsSync: (value) =>
              scenario.existing.some(
                (name) => value === path.join(profileRoot, name),
              ),
          };
...
test("Product identity remains fixed across model selections, themes and viewports", async () => {
  ...
  try {
    ...
    const models = [
      ...
      "gpt-image-1.5",
      "gpt-image-2.5-flare",
      "custom-image-model-with-a-very-long-name",
    ];
    for (const model of models) {
      await page.evaluate((model) => {
        ProviderManager.providers = model
          ? [
              {
                id: "test",
                name: "Test provider",
                model,
                type: model.includes("gemini") ? "gemini" : "openai",
              },
            ]
          : [];
      ...
      for (const id of [
        ...
        assert(
          await page
            .locator(`#${id} img`)
            .evaluate((img) => img.complete && img.naturalWidth === 512),
        );
      }
      const labels = {
        ...
        "custom-image-model-with-a-very-long-name": /custom-image-model/,
      };
      if (model && model !== "random")
        assert.match(
          await page.locator("#empty-brand-subtitle").textContent(),
          labels[model],
        );
    }
    await page.evaluate(() => {
      ...
      for (const mode of ["light", "dark"]) {
        ...
        if (process.env.BRAND_SCREENSHOTS) {
          await page.screenshot({
            path: path.join(
              process.env.BRAND_SCREENSHOTS,
              `genforge-${width}-${mode}.png`,
            ),
            animations: "disabled",
          });
        }
...
test("Packaging and raster assets share the GenForge identity", async () => {
  ...
  const manifest = JSON.parse(
    fs.readFileSync(
      path.join(root, "src/renderer/assets/site.webmanifest"),
      "utf8",
    ),
  );
  ...
  assert.equal(
    ...
    true,
    "Windows executable resources must receive the product name and icon",
  );
  ...
  for (const [file, size] of Object.entries(sizes)) {
    ...
    assert(
      stats.channels.some((channel) => channel.stdev > 40),
      `${file} must contain a nonblank mark`,
    );
  }
});
...
test("Desktop startup reopens legacy settings and conversations in an isolated profile", async () => {
  const temporaryRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "genforge-profile-"),
  );
  ...
  try {
    ...
    assert.equal(await page.title(), "GenForge");
    assert.equal(
      await page.evaluate(() => ProviderManager.providers[0].name),
      "Saved provider",
    );
    assert.equal(await page.evaluate(() => ProviderManager.activeId), "legacy");
    assert.equal(
      await page.evaluate(async () => (await getAllSessions())[0].title),
      "Saved conversation",
    );
    assert.equal(
      await page.evaluate(
        async () => (await getSessionMessages(currentSessionId))[0].content,
      ),
      "Saved message",
    );
    assert.equal(
      await page.locator(".app-select-trigger").count(),
    ...
    assert.equal(
      await page.locator("#image-count").inputValue(),
      "2",
      "Shared popovers must work in the shipped Electron runtime",
    );
    assert(
      !fs.existsSync(path.join(temporaryRoot, "GenForge")),
      "Upgrade must not create an empty replacement profile",
    );
    await desktop.close();
    desktop = undefined;
    ...
    fs.writeFileSync(
      freshEntry,
      `require('electron').app.setPath('appData', ${JSON.stringify(freshRoot)}); require(${JSON.stringify(mainPath)});`,
    );
    desktop = await electron.launch({
      executablePath: require("electron"),
      args: [freshEntry],
