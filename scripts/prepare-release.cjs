...
async function main() {
  ...
  assert(
    ...
    "Only stable X.Y.Z versions are supported",
  );
  ...
  for (const name of required) {
    ...
    assert(
      ...
      `Missing or empty release asset: ${name}`,
    );
  }
  ...
  for (const [name, expected] of Object.entries(updates)) {
    ...
    assert(
      Array.isArray(info.files) &&
        info.files.some((file) => file.url === expected),
      `${name}: missing update package`,
    );
    for (const entry of info.files) {
      assert(
        downloads.some((row) => row[2] === entry.url),
        `${name}: unexpected update URL`,
      );
      const file = path.join(directory, entry.url);
      assert.equal(
        await hash(file, "sha512", "base64"),
        entry.sha512,
        `${name}: hash mismatch`,
      );
    ...
    if (info.path) {
      assert(
        downloads.some((row) => row[2] === info.path),
        `${name}: unexpected legacy update path`,
      );
      assert.equal(
        await hash(path.join(directory, info.path), "sha512", "base64"),
        info.sha512,
        `${name}: legacy hash mismatch`,
      );
    }
  }
  ...
  const blockmaps = fs
    ...
    .filter(
      (name) =>
        name.endsWith(".blockmap") && required.includes(name.slice(0, -9)),
    );
  const assets = [...required, ...blockmaps].sort();
  const checksums = [];
  for (const name of assets)
    checksums.push(
      `${await hash(path.join(directory, name), "sha256", "hex")}  ${name}`,
    );
  fs.writeFileSync(
  ...
  const notes = [
    ...
    "| --- | --- | --- |",
    ...downloads.map(
      ([os, kind, name]) => `| ${os} | ${kind} | [${name}](${base}/${name}) |`,
    ),
    "",
    "## 安装说明 / Installation",
    "",
    "- Windows：推荐 EXE 安装版；便携版和 ZIP 需手动升级。当前构建未配置代码签名。",
    "- macOS：DMG 同时支持 Intel 和 Apple Silicon，打开后拖入「应用程序」并从那里启动。当前构建未签名、未公证；如被系统拦截，请在系统设置的隐私与安全性中允许打开。之后可在应用内更新，退出后会换成新版本再重新打开。",
  ...
  fs.writeFileSync(
    ...
    JSON.stringify([...assets, "SHA256SUMS.txt"]),
  );
